...
which is used for normalization at the end. sm = 0 means: The spline passes
through all grid points. The larger sm (10, 100, 1000....), the stiffer the
spline. After that, you can still delete points that are a certain percentage
larger or smaller than the spline.
...
def wavecalc(flux, header):
    ...
    wave = np.array([header['CRVAL1']+header['CDELT1']*i for i in range(header['NAXIS1'])])
...
files = '/Users/amartinez/Desktop/PhD/KMOS/Kmos_iMac/tramos_reduction/young_candidates/ifu_7/half_2/spec_ifu7_half2_12_52.fits'
...
# #     ll = np.loadtxt('/Users/amartinez/Desktop/PhD/KMOS/practice/lines.txt')

while forderung == "y":
    ll = np.loadtxt('/Users/amartinez/Desktop/PhD/KMOS/practice/lines_ifu7_half2_13_50.txt')
    for j in range(len(ll)):
        loesch1[j] = ll[j][0]
        loesch2[j] = ll[j][1]
        for n in np.arange(len(points_wave)):
            if loesch1[j] < points_wave[n] and loesch2[j] > points_wave[n]:
                points_flux[n] = 0
    forderung = input(
        "Do you want to remove further support points? Then enter y: ")
